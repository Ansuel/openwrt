...
/* SYSCON */
/* HSGMII_ETH_PHY 0x1fa70000 - 7fff */
/* RESERVED 0x1fa78000 - 9fff */
/* XSI_PHY_PMA_PHYA 0x1fa7a000 - afff */
/* XSI_PHY_PMA_PHYD 0x1fa7b000 - bfff */

/* ecnt_eth_phy mode hsgmii */

/* HSGMII_ETH_PHY */
#define HSGMII_SGMII_AN         0x0
#define HSGMII_XSGMII_INIT      0xa00
#define HSGMII_HSGMII_INIT      0x4018
#define HSGMII_HSGMII_25G       0x6000
...
#define PHYA_TXPLL_PCW2		0x94
#define PHYA_TXPLL_PCW1		0x98
...
static int airoha_pcs_setup_hsgmii(struct phylink_pcs *pcs)
{
	// struct airoha_gdm_port *port = container_of(pcs, struct airoha_gdm_port,
	// 					    pcs);

	struct regmap *map_phya, *map_phyd, *map_hsgmii;

	map_phya = syscon_regmap_lookup_by_compatible("airoha,xsi_pma_phya");
	map_phyd = syscon_regmap_lookup_by_compatible("airoha,xsi_pma_phyd");
	map_hsgmii = syscon_regmap_lookup_by_compatible("airoha,hsgmii");

	u32 FL_Out_target = 0xA000; /* no idea */
	/* xsgmii_chg 1 */
	regmap_write(map_phya, 0x0, 0x10040001);
	/* JCPLL BringUp 1 */
	/* JCPLL_LDO */
	regmap_write(map_phya, PHYA_JCPLL_LDO, 0x1020ff);

	/* JCPLL_RSTB */
	regmap_write(map_phya, PHYA_JCPLL_RSTB, 0x3000004);
	regmap_write(map_phya, PHYA_JCPLL_RSTB, 0x3000104);

	/* JCPLL_EN */
	regmap_write(map_phyd, PHYD_JCPLL_EN, 0x1000000);

	/* JCPLL_SDM */
	regmap_write(map_phya, PHYA_JCPLL_RSTB, 0x104);
	regmap_write(map_phya, PHYA_JCPLL_SDM1, 0x30000);
	regmap_write(map_phya, PHYA_JCPLL_SDM2, 0x5010100);

	/* JCPLL_SSC */
	regmap_write(map_phya, PHYA_JCPLL_SSC3, 0x0);
	regmap_write(map_phya, PHYA_JCPLL_SSC2, 0x0);
	regmap_write(map_phya, PHYA_JCPLL_SSC1, 0x3018);

	/* JCPLL_LPF */
	regmap_write(map_phya, PHYA_JCPLL_LPF1, 0x180000);
	regmap_write(map_phya, PHYA_JCPLL_LPF2, 0x101f0a);
	regmap_write(map_phya, PHYA_JCPLL_LPF3, 0x2ff0000);

	/* JCPLL_VCO */
	regmap_write(map_phya, PHYA_JCPLL_VCO, 0x4010100);
	regmap_write(map_phya, PHYA_JCPLL_SSC1, 0x301d);

	/* JCPLL_PCW */
	regmap_write(map_phyd, PHYD_JCPLL_PCW2, 0x25800000);
	regmap_write(map_phyd, PHYD_JCPLL_PCW1, 0x10000);

	/* JCPLL_DIV */
	regmap_write(map_phya, PHYA_JCPLL_DIV, 0x10000);
	regmap_write(map_phya, PHYA_JCPLL_VCO, 0x4010100);

	/* JCPLL_KBand */
	regmap_write(map_phya, PHYA_JCPLL_KBand, 0x1000300);
	regmap_write(map_phya, PHYA_JCPLL_LPF3, 0x2e40000);

	/* JCPLL_TCL */
	regmap_write(map_phya, PHYA_JCPLL_LDO, 0x1020ff);
	regmap_write(map_phya, PHYA_JCPLL_SDM2, 0x5010100);
	regmap_write(map_phya, PHYA_JCPLL_TCL2, 0x10400);

	/* JCPLL_EN */
	regmap_write(map_phyd, PHYD_JCPLL_EN, 0x1010000);

	/* JCPLL_Out */
	regmap_write(map_phyd, PHYD_JCPLL_EN, 0x1010101);

	/* TXPLL BringUp */
	/* TXPLL_VCOLDO_Out */
	regmap_write(map_phya, PHYA_TXPLL_VCOLDO_Out, 0x101031b);

	/* TXPLL_RSTB */
	regmap_write(map_phya, PHYA_TXPLL_RSTB, 0x1040001);

	/* TXPLL_EN */
	regmap_write(map_phyd, PHYD_TXPLL_EN, 0x1000000);

	/* TXPLL_SDM */
	regmap_write(map_phya, PHYA_TXPLL_SDM1, 0x0);
	regmap_write(map_phya, PHYA_TXPLL_SDM2, 0x1000003);

	/* TXPLL_SSC */
	regmap_write(map_phya, PHYA_TXPLL_SSC1, 0x0);
	regmap_write(map_phya, PHYA_TXPLL_SSC2, 0x0);
	regmap_write(map_phya, PHYA_TXPLL_VCOLDO_Out, 0x1010000);

	/* TXPLL_LPF */
	regmap_write(map_phya, PHYA_TXPLL_LPF1, 0x1f05000a);
	regmap_write(map_phya, PHYA_TXPLL_LPF2, 0x5);

	/* TXPLL_VCO */
	regmap_write(map_phya, PHYA_TXPLL_VCO1, 0x1);
	regmap_write(map_phya, PHYA_TXPLL_VCO2, 0x4040701);

	/* TXPLL_PCW */
	regmap_write(map_phya, PHYA_TXPLL_PCW1, 0xa000000);
	regmap_write(map_phya, PHYA_TXPLL_PCW2, 0x1000000);

	/* TXPLL_KBand */
	regmap_write(map_phya, PHYA_TXPLL_KBand1, 0x30004e4);
	regmap_write(map_phya, PHYA_TXPLL_KBand2, 0x101);
	regmap_write(map_phya, PHYA_TXPLL_LPF2, 0x5);

	/* TXPLL_DIV */
	regmap_write(map_phya, PHYA_TXPLL_KBand2, 0x1);
	regmap_write(map_phya, PHYA_TXPLL_VCO1, 0x10001);

	/* TXPLL_TCL */
	regmap_write(map_phya, PHYA_TXPLL_PCW2, 0x1000f);
	regmap_write(map_phya, PHYA_TXPLL_TCL, 0x4000d03);
	regmap_write(map_phya, PHYA_TXPLL_VCO1, 0x10001);
	regmap_write(map_phya, PHYA_TXPLL_SDM2, 0x1000003);

	/* TXPLL_EN */
	regmap_write(map_phyd, PHYD_TXPLL_EN, 0x1010000);

	/* TXPLL_Out */
	regmap_write(map_phyd, PHYD_TXPLL_EN, 0x1010101);

	/* TX BringUp 1 */
	/* tx_rate_ctrl 1 */
	regmap_write(map_phyd, PHYD_tx_rate_ctrl, 0x1);

	/* TX_CONFIG */
	regmap_write(map_phya, PHYA_TX_CONFIG1, 0x1010401);
	regmap_write(map_phyd, PHYD_TX_CONFIG2, 0x1010000);
	regmap_write(map_phyd, PHYD_TX_CONFIG3, 0x1040101);
	regmap_write(map_phyd, PHYD_TX_CONFIG4, 0x101);

	/* TX_FIR_Load_Para swing 2, len 4, cn1 0, c0b b, c1 1, en 1 */
	/* TX_FIR */
	regmap_write(map_phyd, PHYD_TX_FIR1, 0x100010b);
	regmap_write(map_phyd, PHYD_TX_FIR2, 0x101);

	/* TX_RSTB */
	regmap_write(map_phyd, PHYD_TX_RSTB, 0x101);

	/* RX BringUp 1 */
	/* RX_INIT */
	/* rx_rate_ctrl */
	regmap_write(map_phyd, PHYD_rx_rate_ctrl, 0x0);

	/* RX_Path_Init */
	regmap_write(map_phyd, PHYD_RX_Path_Init1, 0x40003ff);
	regmap_write(map_phya, PHYA_RX_Path_Init2, 0x1010101);
	regmap_write(map_phya, PHYA_RX_Path_Init3, 0x1000000);
	regmap_write(map_phya, PHYA_RX_Path_Init4, 0x2000401);
	regmap_write(map_phyd, PHYD_RX_Path_Init5, 0xc100a01);
	regmap_write(map_phya, PHYA_RX_Path_Init6, 0x20000);
	regmap_write(map_phya, PHYA_RX_Path_Init7, 0x3ff08);
	regmap_write(map_phyd, PHYD_RX_Path_Init8, 0x10101);
	regmap_write(map_phyd, PHYD_RX_Path_Init9, 0x1000202);
	regmap_write(map_phya, PHYA_RX_Path_Init10, 0x0);
	regmap_write(map_phyd, PHYD_RX_Path_Init11, 0x1000000);
	regmap_write(map_phyd, PHYD_RX_Path_Init12, 0x1000000);
	regmap_write(map_phya, PHYA_RX_Path_Init13, 0x70604);

	/* RX_FE,force 0, Gain 1, Peaking 1 */
	regmap_write(map_phyd, PHYD_RX_FE1, 0x0);
	regmap_write(map_phyd, PHYD_RX_FE2, 0x0);

	/* RX_FE_VOS */
	regmap_write(map_phyd, PHYD_RX_FE_VOS, 0x10100);

	/* RX_IMP 1 */
	regmap_write(map_phya, PHYA_RX_IMP, 0x1040000);

	/* RX_FLL_PR_FMeter */
	regmap_write(map_phya, PHYA_RX_FLL_PR_FMeter1, 0x100001);
	regmap_write(map_phya, PHYA_RX_FLL_PR_FMeter2, 0xffff0000);
	regmap_write(map_phya, PHYA_RX_FLL_PR_FMeter3, 0x3107);

	/* RX_REV */
	regmap_write(map_phya, PHYA_RX_REV, 0xc8c31030);

	/* RX_Rdy_TimeOut */
	regmap_write(map_phyd, PHYD_RX_Rdy_TimeOut, 0xa0005);

	/* RX_CalBoundry_Init */
	regmap_write(map_phyd, PHYD_RX_CalBoundry_Init1, 0x101);
	regmap_write(map_phyd, PHYD_RX_CalBoundry_Init2, 0x2);
	regmap_write(map_phyd, PHYD_RX_CalBoundry_Init3, 0x320002);
	regmap_write(map_phyd, PHYD_RX_CalBoundry_Init4, 0x320002);
	regmap_write(map_phyd, PHYD_RX_CalBoundry_Init5, 0x320002);
	regmap_write(map_phyd, PHYD_RX_CalBoundry_Init6, 0x320002);

	/* RX_BySerdes */
	/* RX_OSR */
	regmap_write(map_phyd, PHYD_RX_OSR, 0x1010000);
	regmap_write(map_phya, PHYA_RX_Path_Init10, 0x100);

	/* CDR_LPF_RATIO */
	regmap_write(map_phya, PHYA_CDR_LPF_RATIO, 0x2000001);

	/* CDR_PR */
	regmap_write(map_phya, PHYA_CDR_PR1, 0x4010806);
	regmap_write(map_phya, PHYA_CDR_PR2, 0x60606);

	/* RX_EYE_Mon */
	regmap_write(map_phyd, PHYD_RX_EYE_Mon1, 0x103);
	regmap_write(map_phyd, PHYD_RX_EYE_Mon2, 0x1);

	/* RX_SYS_En */
	regmap_write(map_phyd, PHYD_RX_SYS_En1, 0x1);
	regmap_write(map_phyd, PHYD_RX_SYS_En2, 0x1000000);

	/* RX_FLL_PR_FMeter */
	regmap_write(map_phyd, PHYD_RX_FLL_PR_FMeter1, 0x1010100);
	regmap_write(map_phyd, PHYD_RX_FLL_PR_FMeter2, 0x1);

	/* RX_CMLEQ_EN */
	regmap_write(map_phya, PHYA_RX_CMLEQ_EN, 0x1010100);

	/* RX_CDR_PR */
	regmap_write(map_phya, PHYA_RX_Path_Init13, 0xe0604);

	/* RX_CDR_xxx_Pwdb */
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb1, 0x1000100);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb2, 0x100);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb3, 0x100);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb4, 0x1000000);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb5, 0x0);

	/* RX_SigDet */
	regmap_write(map_phya, PHYA_RX_IMP, 0x1020200);
	regmap_write(map_phya, PHYA_RX_SigDet, 0x3000200);

	/* RX_SigDet_Pwdb */
	regmap_write(map_phyd, PHYD_RX_SigDet_Pwdb, 0x0);

	/* PXP_RX_PHYCK */
	regmap_write(map_phya, PHYA_PXP_RX_PHYCK1, 0x1010b);
	regmap_write(map_phya, PHYA_PXP_RX_PHYCK2, 0x1000000);

	/* RX_CDR_xxx_Pwdb */
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb1, 0x1010101);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb2, 0x101);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb3, 0x101);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb4, 0x1010000);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb5, 0x1010101);

	/* RX_SigDet_Pwdb */
	regmap_write(map_phyd, PHYD_RX_SigDet_Pwdb, 0x1);

	/* RX_PR_CAL_SEQ 1 */
	/* RX_CDR_LFP_L2D mode 1, sel 0*/
	regmap_write(map_phyd, PHYD_RX_CDR_LFP_L2D, 0x100);

	/* RX_PRCal 1 */
	regmap_write(map_phyd, PHYD_RX_PRCal1, 0x20);
	regmap_write(map_phyd, PHYD_RX_PRCal2, 0xa0649f9c);
	regmap_write(map_phyd, PHYD_RX_PRCal3, 0x4e204e20);
	regmap_write(map_phyd, PHYD_RX_PRCal4, 0x3307);
	regmap_write(map_phyd, PHYD_RX_PRCal5, 0xa0649f9c);
	regmap_write(map_phya, PHYA_RX_PRCal6, 0x1000000);
	regmap_write(map_phyd, PHYD_RX_PRCal7, 0x1010100);
	regmap_write(map_phyd, PHYD_RX_PRCal8, 0x1010000);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb1, 0x1010101);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb1, 0x1000101);
	regmap_write(map_phyd, PHYD_RX_CDR_xxx_Pwdb1, 0x1010101);

	unsigned int PrCal_Serach = 0;
	...
	for (PrCal_Serach = 1; PrCal_Serach < 8 ; PrCal_Serach++) {
		...
		regmap_update_bits(map_phyd, PHYD_RX_PRCal8,
				GENMASK(10, 0),
				FIELD_PREP(GENMASK(10, 0), PrCal_Serach << 8));
		
		regmap_write(map_phyd, PHYD_RX_PRCal4, 0x3300);
		regmap_write(map_phyd, PHYD_RX_PRCal4, 0x3303);

		usleep_range(5000, 7000);

		regmap_read(map_phyd, PHYD_RX_FREQDET, &val);

		unsigned int RO_FL_Out = FIELD_GET(GENMASK(31, 16), val);
		unsigned int RO_FL_Out_diff;
	...
	regmap_clear_bits(map_phya, PHYA_RX_PRCal6, BIT(24));

	regmap_update_bits(map_phyd, PHYD_RX_PRCal7,
			BIT(0) | BIT(8) | BIT(16) | BIT(24),
			BIT(16) | BIT(24));

	regmap_clear_bits(map_phyd, PHYD_RX_PRCal8, BIT(16));

	regmap_set_bits(map_phyd, PHYD_SS_RX_FLL_b, BIT(0));

	regmap_set_bits(map_phyd, PHYD_SS_RX_FLL_1, cdr_pr_idac_tmp);

	regmap_clear_bits(map_phyd, PHYD_RX_CDR_xxx_Pwdb1, BIT(16));

	regmap_set_bits(map_phyd, PHYD_RX_CDR_xxx_Pwdb1, BIT(16));

	regmap_clear_bits(map_phyd, PHYD_RX_CDR_xxx_Pwdb1, BIT(24));

	regmap_write(map_phyd, PHYD_RX_PRCal1, BIT(16));

	/* RX_PRCal_end */

	/* RX_CDR_LPF_RSTB mode 1, sel0 */
	regmap_write(map_phyd, PHYD_RX_CDR_LFP_L2D, 0x1000100);

	usleep_range(700, 1000);

	/* RX_CDR_LPF_RSTB mode 1, sel1 */
	regmap_write(map_phyd, PHYD_RX_CDR_LFP_L2D, 0x1010100);

	usleep_range(100, 200);

	/* RX_CDR_LFP_L2D mode 1, sel 1 */
	regmap_write(map_phyd, PHYD_RX_CDR_LFP_L2D, 0x1010101);

	/* RX_CDR_LPF_RSTB mode 0, sel1 */
	regmap_write(map_phyd, PHYD_RX_CDR_LFP_L2D, 0x10101);
	regmap_write(map_phyd, PHYD_RX_CDR_LFP_L2D, 0x10001);

	/* RSTB sel8, val 0 */
	regmap_write(map_phyd, PHYD_RX_PRCal1, 0x20);
	/* RSTB sel8, val 1 */
	regmap_write(map_phyd, PHYD_RX_PRCal1, 0xfff);

	/* RX_CDR_RST */

	/* RX_CDR_LFP_L2D mode 1, sel 0 */
	regmap_write(map_phyd, PHYD_RX_CDR_LFP_L2D, 0x10100);
	/* RX_CDR_LPF_RSTB mode 1, sel0 */
	regmap_write(map_phyd, PHYD_RX_CDR_LFP_L2D, 0x1000100);
	...
	/* xsgmii_init */
	/* usxgmii_init */
	regmap_write(map_hsgmii, HSGMII_XSGMII_INIT, 0x4c9cc000);
	regmap_write(map_hsgmii, HSGMII_HSGMII_INIT, 0x34);

	/* hsgmii_init */
	/* xsgmii 1 int enable 0 */
	/* force mode */
	/* xSGMII_AN_API 1, enable : 0 */
	/* sgmii_an 0 */
	regmap_write(map_hsgmii, HSGMII_SGMII_AN, 0x140);

	/* hsgmii */
	/* HGMII_2.5G */
	regmap_write(map_hsgmii, HSGMII_HSGMII_25G, 0xc000c11);
